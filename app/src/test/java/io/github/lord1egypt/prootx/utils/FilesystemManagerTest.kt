...
class FilesystemManagerTest {
    ...
    fun setup() {
        ...
        whenever(mockProotXFiles.filesDir).thenReturn(tempFolder.root)
        filesystemManager = FilesystemManager(mockProotXFiles, mockBusyboxExecutor, mockLogger)
    }
    ...
    fun `copyAssetsToFilesystem copies to the right directory, including rootfs files if filesystem is not from backup`() {
        ...
        targetFiles.forEach { file ->
            verify(mockProotXFiles).makePermissionsUsable(targetDirectory.path, file.name)
        }
    }
    ...
    fun `copyAssetsToFilesystem copies to the right directory, excluding rootfs files if filesystem is from backup`() {
        ...
        targetFiles.forEach {
            ...
            verify(mockProotXFiles).makePermissionsUsable(targetDirectory.path, it.name)
        }
    }
