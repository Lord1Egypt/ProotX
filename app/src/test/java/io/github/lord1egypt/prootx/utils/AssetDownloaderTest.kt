...
class AssetDownloaderTest {
    ...
    @Mock lateinit var mockProotXFiles: ProotXFiles
    ...
    fun setup() {
        ...
        whenever(mockProotXFiles.filesDir).thenReturn(mockFilesDir)
        whenever(mockProotXFiles.emulatedScopedDir).thenReturn(mockScopedStorageDir)
        ...
        assetDownloader = AssetDownloader(assetPreferences, downloadManagerWrapper, mockProotXFiles)
    }
    ...
    @Test
    fun `Returns NonProotXDownloadFound if a a download we did not start is found`() {
        setupDownloadState()

        val result = assetDownloader.handleDownloadComplete(-1)

        assertTrue(result is NonProotXDownloadFound)
    }
    ...
    @Test
    fun `Clears download directory of prootx files`() {
