...
class BusyboxExecutor(
    ...
    private val busyboxWrapper: BusyboxWrapper = BusyboxWrapper(prootxFiles)
) {
    ...
    private fun runCommand(command: List<String>, listener: (String) -> Any): ExecutionResult {
        ...
        processBuilder.directory(prootxFiles.filesDir)
    ...
    fun executeProotCommand(
        ...
        val filesystemDir = File("${prootxFiles.filesDir.absolutePath}/$filesystemDirName")
...
class BusyboxWrapper(private val prootxFiles: ProotXFiles) {
    // For basic commands, CWD should be `applicationFilesDir`
    fun wrapCommand(command: String): List<String> {
        return listOf(prootxFiles.busybox.path, "sh", "-c", command)
    }

    fun wrapScript(command: String): List<String> {
        return listOf(prootxFiles.busybox.path, "sh") + command.split(" ")
    }
    ...
    fun busyboxIsPresent(): Boolean {
        return prootxFiles.busybox.exists()
    }

    // Proot scripts expect CWD to be `applicationFilesDir/<filesystem`
    fun addBusyboxAndProot(command: String): List<String> {
        return listOf(prootxFiles.busybox.absolutePath, "sh", "support/execInProot.sh") + command.split(" ")
    }
    ...
    fun getProotEnv(filesystemDir: File, prootDebugLevel: String): HashMap<String, String> {
        ...
        val emulatedStorageBinding = "-b ${prootxFiles.emulatedUserDir.absolutePath}:/storage/internal"
        val externalStorageBinding = prootxFiles.sdCardUserDir?.run {
    ...
    fun prootIsPresent(): Boolean {
        return prootxFiles.proot.exists()
    }

    fun executionScriptIsPresent(): Boolean {
        val execInProotFile = File(prootxFiles.supportDir, "execInProot.sh")
