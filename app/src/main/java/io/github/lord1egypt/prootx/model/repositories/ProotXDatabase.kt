...
abstract class ProotXDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        @Volatile
        private var INSTANCE: ProotXDatabase? = null

        fun getInstance(context: Context): ProotXDatabase =
                INSTANCE ?: synchronized(this) {
                    INSTANCE
                            ?: buildDatabase(context).also { INSTANCE = it }
                }

        private fun buildDatabase(context: Context): ProotXDatabase =
                Room.databaseBuilder(context.applicationContext,
                        ProotXDatabase::class.java, "Data.db")
                        .addMigrations(
                                Migration1To2(),
                                Migration2To3(),
                                Migration3To4(),
...
class Migration2To3 : Migration(2, 3) {
    override fun migrate(database: SupportSQLiteDatabase) {
        ...
        database.execSQL("ALTER TABLE filesystem ADD COLUMN defaultVncPassword TEXT NOT NULL DEFAULT 'prootx'")
        database.execSQL("ALTER TABLE session ADD COLUMN vncPassword TEXT NOT NULL DEFAULT 'prootx'")
    }
}
