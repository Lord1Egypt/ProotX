package io.github.lord1egypt.prootx.model.state
...
import io.github.lord1egypt.prootx.model.entities.Asset
    ...
    fun transitionIsAcceptable(event: SessionStartupEvent): Boolean {
        ...
        return when (event) {
            ...
            is AssetDownloadComplete -> {
                ...
                currentState is DownloadingAssets || !assetDownloader.downloadIsForProotX(event.downloadAssetId)
            }
...
//                currentState is WaitingForSessionSelection || currentState is (DownloadingAssets)
    ...
    fun submitEvent(event: SessionStartupEvent, coroutineScope: CoroutineScope) = coroutineScope.launch {
        val eventBreadcrumb = ProotXBreadcrumb(className, BreadcrumbType.ReceivedEvent, "Event: $event State: ${state.value}")
