package io.github.lord1egypt.prootx.provider
...
class ProotXDocProvider : DocumentsProvider() {
    ...
    private val prootxFiles by lazy {
        ProotXFiles(context!!, context!!.applicationInfo.nativeLibraryDir)
    }
    ...
    private fun addProotXRoots(result: MatrixCursor): Cursor {
        val baseEmulatedDir = prootxFiles.emulatedUserDir
        result.newRow().apply {
            add(Root.COLUMN_TITLE, context!!.getString(R.string.app_name) + " INTERNAL")
            // Root for ProotX storage should be the files dir
        ...
        val baseSdCardDir = prootxFiles.sdCardUserDir
        baseSdCardDir?.let {
            result.newRow().apply {
