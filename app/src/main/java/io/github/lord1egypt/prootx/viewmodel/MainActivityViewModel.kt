...
class MainActivityViewModel(
    ...
    init {
        state.addSource(appsState) { it?.let { update ->
            val breadcrumb = ProotXBreadcrumb(className, BreadcrumbType.ObservedState, "$update")
    ...
    private fun submitAppsStartupEvent(event: AppsStartupEvent) {
        val breadcrumb = ProotXBreadcrumb(className, BreadcrumbType.SubmittedEvent, "$event")
        logger.addBreadcrumb(breadcrumb)
